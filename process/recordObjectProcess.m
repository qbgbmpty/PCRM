function recordObjectProcess(image_num,objnum,ObjectProcess)
    ...
    for image = 1:1:image_num-1
        ...
        if recordNum(obj,image) == recordNum(obj,image+1)
            if recordNum(obj,image) == 3 && (ObjectProcess(1,image,obj) == ObjectProcess(2,image,obj) || ObjectProcess(2,image,obj) == ObjectProcess(3,image,obj) || ObjectProcess(1,image,obj) == ObjectProcess(3,image,obj))
                ...
                f_recordRelationFileName = OneOfRelationFileName(temp_image);
                [f_sym f_former f_later f_fx f_fy f_lx f_ly] = textread([pwd,'/',f_recordRelationFileName,'.txt'],'%c %d %d %d %d %d %d');
...
ObjectFileName = ['TrackingProcess/recordObjectProcess/ObjectProcess'];
